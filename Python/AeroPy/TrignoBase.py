...
import sys
sys.path.append(r'C:\Users\Elisa\Downloads\notte_ricerca_emg\Python')
...
class TrignoBase():
    ...
    def PipelineState_Callback(self):
        return self.TrigBase.GetPipelineState() #Returns the current pipeline state of the Trigno Base (e.g., "Connected", "Armed")

    def Connect_Callback(self):
        """Callback to connect to the base"""
        self.TrigBase.ValidateBase(key, license) #Authenticates and connects to the Trigno Base using a key and license

    def Pair_Callback(self):
        return self.TrigBase.PairSensor(self.pair_number) #Pairs a specific sensor with the Trigno Base
    ...
    def Start_Callback(self, start_trigger, stop_trigger):
        ...
        if configured:
            ...
            self.TrigBase.Start(self.collection_data_handler.streamYTData) #start real-time data streaming
    ...
    def ConfigureCollectionOutput(self):
        ...
        if self.TrigBase.GetPipelineState() == 'Armed': #ready for streaming
        ...
        elif self.TrigBase.GetPipelineState() == 'Connected': #needs configuration
    ...
    def Stop_Callback(self):
        ...
        self.TrigBase.Stop() #stop the data stream
