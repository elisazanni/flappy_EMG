...
# Define host and ports for each sensor type (change ports as needed)
HOST = '127.0.0.1'
INFO_PORT = 9001
EMG_PORT = 9002
ACC_PORT = 9003
...
# Connect sockets (make sure server is listening on these ports)
info_socket.connect((HOST, INFO_PORT))
emg_socket.connect((HOST, EMG_PORT))
acc_socket.connect((HOST, ACC_PORT))
gyro_socket.connect((HOST, GYRO_PORT))
...
def process_channel_data(channel_guid, data):
    ...
    if sensor_type is None:
        # Skip if not recognized sensor type
        return
    ...
    for sample in data:
        ...
        # Send data over the socket as a CSV line
        message = f"{sensor_type},{channel_guid},{time_point},{value}\n"
        try:
            sock.sendall(message.encode())
        except Exception as e:
            print(f"Socket send error for {sensor_type}: {e}")
...
if __name__ == "__main__":
    ...
    if base: 
        ...
        for sensor in sensors:
            # print(f"Component Number: {sensor.Id}")
            message1 = f"Component Number: {sensor.Id} \n"
            try:
                info_socket.sendall(message1.encode())
            except Exception as e:
                print(f"Socket send error for {message1}: {e}")
            ...
            for channel in sensor.TrignoChannels:
                ...
                message2 = f"  Channel Name: {channel.Name}, GUID: {channel.Id}, Type: {channel.Type}"
                try:
                    info_socket.sendall(message2.encode())
                except Exception as e:
                    print(f"Socket send error for {message2}: {e}")
